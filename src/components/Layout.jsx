  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className={`border-b border-slate-800 flex items-center justify-between shrink-0 ${collapsed ? 'px-3 py-6' : 'px-5 py-6'}`}>
        {collapsed ? (
          <span className="text-amber-400 font-black text-xl mx-auto">A</span>
        ) : (
          <>
            <div>
              <h1 className="text-lg font-bold text-amber-400">ABMDI Registry</h1>
...
export default function Layout({ screen, onNavigate, children, lightMode, onToggleTheme, notifEnabled, onToggleNotif }) {
  ...
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('abmdi_sidebar_collapsed') === '1');

  function toggleCollapse() {
    const next = !collapsed;
    setCollapsed(next);
    localStorage.setItem('abmdi_sidebar_collapsed', next ? '1' : '0');
  }

  const currentLabel = NAV_GROUPS.flatMap(g => g.items).find(i => i.id === screen)?.label || 'ABMDI Registry';
