...
export default function StudyMode({ onNavigate }) {
  const { recordAnswer, getDueQuestions, progress } = useProgress();
  ...
  if (!card) {
    return (
      <Screen title="Flashcards" onBack={() => onNavigate('home')}>
        <div className="flex flex-col items-center justify-center flex-1 gap-4 p-6">
          <p className="text-4xl">🎉</p>
          <p className="text-xl font-bold">No cards due!</p>
          <p className="text-slate-400 text-sm text-center">All caught up. Try switching to "All Cards" mode.</p>
          <button onClick={() => setMode('all')} className="btn-primary">Show All Cards</button>
        </div>
      </Screen>
    );
  }
  ...
  return (
    <Screen title="Flashcards" onBack={() => onNavigate('home')}>
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-800">
        ...
        <button
          onClick={() => setMode(m => m === 'all' ? 'due' : 'all')}
          className={`text-xs px-3 py-2 rounded-lg font-medium transition-colors ${
            mode === 'due' ? 'bg-amber-500 text-slate-900' : 'bg-slate-800 text-slate-300'
          }`}
        >
          {mode === 'due' ? '⏰ Due' : '📚 All'}
        </button>
      ...
      <div className="flex items-center justify-between px-4 py-2 text-xs text-slate-400">
        ...
        <span>{sessionTotal > 0 && `${sessionCorrect}/${sessionTotal} correct`}</span>
      </div>
      <div className="mx-4 h-1 bg-slate-800 rounded-full overflow-hidden mb-4">
      ...
      <div className="flex-1 flex flex-col px-4 pb-4">
        <div
          className="card-flip-container flex-1 cursor-pointer"
        ...
        {showResult && (
          <div className={`mt-4 py-4 rounded-2xl text-center font-bold text-lg ${
            showResult === 'correct' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
          }`}>
        ...
        {!flipped && (
          <button
            onClick={() => { setFlipped(false); setCardIndex(i => (i + 1) % deck.length); }}
          ...
          {topics.map(t => (
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl text-sm transition-colors ${
                selected === t ? 'bg-amber-500/20 text-amber-300 font-semibold' : 'hover:bg-slate-700 text-slate-300'
              }`}
            >
              {t}
            </button>
